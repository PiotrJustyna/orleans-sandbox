...
namespace OrleansSandbox.Grains
{
  ...
  public class GameGrain : Grain, IGameGrain
  {
    ...
    private readonly HashSet<IGameObserver> _observers = new HashSet<IGameObserver>();
      ...
      foreach (var player in _status.PlayerKeys)
      {
        if (!_players.Contains(player))
        {
          try
          {
            ...
            await GrainFactory
              .GetGrain<IPlayerGrain>(player)
              .JoinGameAsync(this.AsReference<IGameGrain>());
        ...
        if (!_status.PlayerKeys.Contains(player))
        {
          ...
          promises.Add(
            Tuple.Create(
              ...
              GrainFactory
                .GetGrain<IPlayerGrain>(player)
                .LeaveGameAsync(this.AsReference<IGameGrain>())
            ));
        }
      }
